...
- **Label** -- The display label shown to users (e.g., "Phone Number", "Department"). You can customize labels to match your organization's terminology.

The field's internal key (e.g., `telephoneNumber`, `department`) is shown next to the toggle for reference.

### Saving Changes

After toggling fields or editing labels, click **Save Fields** to apply the changes. The self-service portal updates immediately for all users.
...
A strong password is generated automatically and stored encrypted on the server. The password is never displayed to anyone.
